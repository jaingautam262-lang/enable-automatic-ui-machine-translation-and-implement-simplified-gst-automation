import { ExportValidationError, ExportValidationSummary, ExportType } from './types';
...
function validateEInvoice(data: any, errors: ExportValidationError[], warnings: ExportValidationError[]) {
  if (!data.invoiceNumber) {
    errors.push({ field: 'invoiceNumber', message: 'Invoice number is required', severity: 'error' });
  }
  if (!data.supplierGSTIN) {
    errors.push({ field: 'supplierGSTIN', message: 'Supplier GSTIN is required', severity: 'error' });
  }
  if (!data.customerGSTIN) {
    errors.push({ field: 'customerGSTIN', message: 'Customer GSTIN is required', severity: 'error' });
  }
  if (!data.items || data.items.length === 0) {
    errors.push({ field: 'items', message: 'At least one item is required', severity: 'error' });
  }
}

function validateEWayBill(data: any, errors: ExportValidationError[], warnings: ExportValidationError[]) {
  if (!data.invoiceNumber) {
    errors.push({ field: 'invoiceNumber', message: 'Invoice number is required', severity: 'error' });
  }
  if (!data.fromGSTIN) {
    errors.push({ field: 'fromGSTIN', message: 'From GSTIN is required', severity: 'error' });
  }
  if (!data.toGSTIN) {
    errors.push({ field: 'toGSTIN', message: 'To GSTIN is required', severity: 'error' });
  }
  if (!data.transportMode) {
    errors.push({ field: 'transportMode', message: 'Transport mode is required', severity: 'error' });
  }
  if (!data.distance || data.distance <= 0) {
    errors.push({ field: 'distance', message: 'Valid distance is required', severity: 'error' });
  }
}
...
function validateGSTR3B(data: any, errors: ExportValidationError[], warnings: ExportValidationError[]) {
  if (!data.period) {
    errors.push({ field: 'period', message: 'Period is required (MMYYYY format)', severity: 'error' });
  }
  if (!data.outwardSupplies) {
    errors.push({ field: 'outwardSupplies', message: 'Outward supplies data is required', severity: 'error' });
  }
  if (!data.inwardSupplies) {
    errors.push({ field: 'inwardSupplies', message: 'Inward supplies data is required', severity: 'error' });
  }
}
