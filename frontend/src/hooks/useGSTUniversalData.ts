...
function saveStoredRecords(principal: string, records: UnifiedGSTRecord[]): void {
  const key = `${STORAGE_KEY}_${principal}`;
...
export function useGetGSTRecord(principal: string | undefined, recordId: string | undefined) {
...
export function useCreateGSTRecord(principal?: string) {
  ...
  return useMutation({
    mutationFn: async (record: Omit<UnifiedGSTRecord, 'id' | 'owner' | 'createdAt' | 'updatedAt'>) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['gstUnifiedRecords', principal] });
    },
  });
}
...
export function useUpdateGSTRecord(principal?: string) {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<UnifiedGSTRecord> }) => {
      if (!principal) throw new Error('User not authenticated');
