...
import { AutomationStatus, AutomationRunLog, GSPProvider } from '../types/gst-universal';
...
function updateAutomationStatus(principal: string, status: Partial<AutomationStatus>): void {
...
function checkAnyProviderConfigured(principal?: string): boolean {
  ...
  const providers: GSPProvider[] = ['cleartax', 'mastersIndia', 'iris', 'tally'];
  return providers.some((provider) => {
    const creds = getStoredCredentials(principal, provider);
    return creds && creds.enabled && creds.apiKey && creds.clientId;
  });
}
  ...
  return useMutation({
    mutationFn: async (operationType: AutomationRunLog['operationType']) => {
      ...
      if (!checkAnyProviderConfigured(principal)) {
        throw new Error('No GSP provider configured. Please configure at least one provider in Settings.');
      }
