...
import { WorkflowDefinition, WorkflowRunLog, WorkflowStepLog } from '../types/gst-workflows';
...
function saveStoredWorkflows(principal: string, workflows: WorkflowDefinition[]): void {
  const key = `${WORKFLOWS_KEY}_${principal}`;
...
export function useGetWorkflow(principal: string | undefined, workflowId: string | undefined) {
...
export function useCreateWorkflow(principal?: string) {
  ...
  return useMutation({
    mutationFn: async (workflow: Omit<WorkflowDefinition, 'id' | 'owner' | 'createdAt' | 'updatedAt'>) => {
...
export function useRunWorkflow(principal?: string) {
  ...
  return useMutation({
    mutationFn: async (workflowId: string) => {
      ...
      // Update step logs
      const completedStepLogs: WorkflowStepLog[] = workflow.steps.map((step, idx) => ({
        stepId: step.id,
        stepName: step.name,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['gstWorkflowRuns', principal] });
      queryClient.invalidateQueries({ queryKey: ['gstAutomationLogs', principal] });
    },
  });
}
