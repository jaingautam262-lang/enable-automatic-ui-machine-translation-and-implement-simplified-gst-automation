...
export function getStoredCredentials(principal?: string, provider: GSPProvider = 'cleartax'): ProviderCredentials | null {
...
function saveStoredCredentials(principal: string, provider: GSPProvider, credentials: ProviderCredentials): void {
  const key = `${STORAGE_KEY_PREFIX}_${provider}_${principal}`;
  localStorage.setItem(key, JSON.stringify(credentials));
}

function deleteStoredCredentials(principal: string, provider: GSPProvider): void {
  const key = `${STORAGE_KEY_PREFIX}_${provider}_${principal}`;
  localStorage.removeItem(key);
}

export function useGetProviderCredentials(principal?: string, provider: GSPProvider = 'cleartax') {
...
export function useUpsertProviderCredentials(principal?: string, provider: GSPProvider = 'cleartax') {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['providerCredentials', provider, principal] });
      queryClient.invalidateQueries({ queryKey: ['gstAutomationStatus', principal] });
    },
  });
}

export function useDeleteProviderCredentials(principal?: string, provider: GSPProvider = 'cleartax') {
  const queryClient = useQueryClient();
