...
import { safeLocalStorage, dateToTime } from '../lib/serialization';
...
export function useTDSTCSPersistence() {
  ...
  const tdsQuery = useQuery<TDSTransaction[]>({
    ...
    queryFn: async () => {
      ...
      return safeLocalStorage.getItem<TDSTransaction[]>(storageKey, BIGINT_FIELDS.tds) || [];
    },
  ...
  const tcsQuery = useQuery<TCSRecord[]>({
    ...
    queryFn: async () => {
      ...
      return safeLocalStorage.getItem<TCSRecord[]>(storageKey, BIGINT_FIELDS.tcs) || [];
    },
    enabled: !!actor && !isFetching && !!identity,
    staleTime: 30000,
  });
  ...
  const createTDSMutation = useMutation({
    mutationFn: async (data: Omit<TDSTransaction, 'id' | 'owner' | 'date'>) => {
      ...
      const storageKey = `tdsTransactions_${principalKey}`;
      const records = safeLocalStorage.getItem<TDSTransaction[]>(storageKey, BIGINT_FIELDS.tds) || [];
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tdstcs', 'tds', principalKey] });
    },
  });
  ...
  const createTCSMutation = useMutation({
    mutationFn: async (data: Omit<TCSRecord, 'id' | 'owner' | 'date'>) => {
      ...
      const storageKey = `tcsRecords_${principalKey}`;
      const records = safeLocalStorage.getItem<TCSRecord[]>(storageKey, BIGINT_FIELDS.tcs) || [];
      records.push(newRecord);
      safeLocalStorage.setItem(storageKey, records);
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tdstcs', 'tcs', principalKey] });
    },
  });
  ...
  return {
    ...
    updateTDS: (id: bigint, data: TDSTransaction) => updateTDSMutation.mutateAsync({ id, data }),
    deleteTDS: deleteTDSMutation.mutateAsync,
    createTCS: createTCSMutation.mutateAsync,
    updateTCS: (id: bigint, data: TCSRecord) => updateTCSMutation.mutateAsync({ id, data }),
