...
interface Asset {
  ...
  name: string;
  ipAddress: string;
  type: 'server' | 'workstation' | 'network-device' | 'iot' | 'container' | 'cloud';
  os: string;
...
interface SecurityAlert {
  id: string;
  type: 'vulnerability' | 'malware' | 'configuration' | 'compliance' | 'anomaly' | 'policy-violation';
  severity: 'critical' | 'high' | 'medium' | 'low';
  title: string;
  description: string;
  affectedAssets: string[];
...
interface ScanConfiguration {
  id: string;
  name: string;
  type: 'network-discovery' | 'vulnerability' | 'configuration' | 'malware' | 'compliance' | 'policy-audit';
...
export default function SecurityComplianceScanTab() {
  ...
  const [selectedControlGroup, setSelectedControlGroup] = useState<string>('all');
  ...
  const [selectedGapFramework, setSelectedGapFramework] = useState<ComplianceFramework | null>(null);
  ...
  const [vulnerabilities] = useState<Vulnerability[]>([
    {
      ...
      description: 'Critical SQL injection vulnerability allowing remote code execution',
      cvssScore: 9.8,
  ...
  const [configIssues] = useState<ConfigurationIssue[]>([
    {
      ...
      recommendation: 'Disable password authentication and root login. Use key-based authentication only.',
      status: 'open',
    },
    {
      id: 'c2',
      ...
      recommendation: 'Create dedicated database user with minimal required privileges.',
      status: 'open',
    },
  ]);
      ...
      gapAnalysis: [
        {
          ...
          priority: 'high',
          remediation: 'Implement automated identity lifecycle management system',
      ...
      gapAnalysis: [
        {
          ...
          priority: 'critical',
          remediation: 'Change all default passwords and implement password policy',
  ...
  const [auditLogs] = useState<AuditLog[]>([
    {
      ...
      severity: 'info',
      description: 'Completed vulnerability scan on production-servers control group',
      user: 'system',
  ...
  const averageComplianceScore = Math.round(
    complianceFrameworks.reduce((sum, f) => sum + f.complianceScore, 0) / complianceFrameworks.length
  );
  const activeAlerts = securityAlerts.filter((a) => a.status === 'new').length;
  const totalControlGroups = controlGroups.length;
  const totalAssetsInGroups = controlGroups.reduce((sum, cg) => sum + cg.assetCount, 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Enterprise-scale security scanning with policy auditing and compliance verification
          </p>
        </div>
        <img
        ...
        <TabsContent value="dashboard" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Across {totalControlGroups} control groups</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                ...
                <p className="text-xs text-destructive">{criticalVulnerabilities} Critical</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {Math.round(policyAudits.reduce((sum, p) => sum + p.complianceScore, 0) / policyAudits.length)}%
                </div>
                <p className="text-xs text-muted-foreground">{policyAudits.length} policies audited</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Real-time security metrics and risk assessment</CardDescription>
              </CardHeader>
              <CardContent>
                <img
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold text-destructive">High (7.2/10)</span>
                    </div>
                    <Progress value={72} className="h-2" />
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Patch Coverage</span>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Configuration Compliance</span>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Start security scans and assessments</CardDescription>
              </CardHeader>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="flex items-center gap-2">
                        ...
                        Scanning...
                        {asyncScanEnabled && <Badge variant="outline">Async</Badge>}
                        {performanceOptimization && <Badge variant="outline">Optimized</Badge>}
                      </span>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest security findings requiring attention</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-1 flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {formatDate(alert.timestamp)} • Affected: {alert.affectedAssets.join(', ')}
                            </p>
                          </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Discovered devices and endpoints across your network</CardDescription>
                </div>
                <div className="flex gap-2">
                  <Select value={selectedControlGroup} onValueChange={setSelectedControlGroup}>
                ...
                <TableBody>
                  {assets.map((asset) => (
                    <TableRow key={asset.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary">{asset.controlGroup || 'Unassigned'}</Badge>
                      </TableCell>
                      <TableCell>{getStatusBadge(asset.status)}</TableCell>
                      <TableCell>
                        <Badge variant={asset.vulnerabilityCount > 5 ? 'destructive' : 'default'}>
                          {asset.vulnerabilityCount}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant={asset.riskScore > 7 ? 'destructive' : asset.riskScore > 4 ? 'default' : 'secondary'}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Identified security weaknesses and CVEs</CardDescription>
                </div>
                <div className="flex gap-2">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Security configuration issues and hardening recommendations</CardDescription>
                </div>
                      ...
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="space-y-1 flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">Asset: {issue.assetName}</p>
                          </div>
                        </div>
                      </CardHeader>
                      <CardContent>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Compliance assessment with gap analysis for major frameworks</CardDescription>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={() => handleExportReport('pdf', 'Compliance')}>
                    <Download className="mr-2 h-4 w-4" />
                    Export PDF
                  </Button>
                  <Button variant="outline" onClick={() => handleExportReport('excel', 'Compliance')}>
                    <Download className="mr-2 h-4 w-4" />
                    Export Excel
                  </Button>
                </div>
                  ...
                  <Card key={framework.id}>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="text-3xl font-bold">{framework.complianceScore}%</div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <div className="text-2xl font-bold">{framework.totalControls}</div>
                          <p className="text-sm text-muted-foreground">Total Controls</p>
                        </div>
                        <div>
                          <div className="text-2xl font-bold text-green-600">{framework.passedControls}</div>
                          <p className="text-sm text-muted-foreground">Passed</p>
                        </div>
                        <div>
                          <div className="text-2xl font-bold text-destructive">{framework.failedControls}</div>
                          <p className="text-sm text-muted-foreground">Failed</p>
                        </div>
                      </div>
                      <Separator />
                      <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <span>Last Assessment: {formatDate(framework.lastAssessment)}</span>
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => handleViewGapAnalysis(framework)}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Internal organizational policy compliance with pass/fail indicators</CardDescription>
                </div>
                <div className="flex gap-2">
                  <Button onClick={() => setShowPolicyAuditDialog(true)} variant="outline">
                  ...
                  <Button variant="outline" onClick={() => handleExportReport('pdf', 'Policy Audit')}>
                    <Download className="mr-2 h-4 w-4" />
                    Export PDF
                  </Button>
                  <Button variant="outline" onClick={() => handleExportReport('excel', 'Policy Audit')}>
                    <Download className="mr-2 h-4 w-4" />
                    Export Excel
                  </Button>
                </div>
                  ...
                  <Card key={policy.id}>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="text-3xl font-bold">{policy.complianceScore}%</div>
                          <p className="text-sm text-muted-foreground">Compliance</p>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <Progress value={policy.complianceScore} className="h-3" />
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <div className="text-2xl font-bold">{policy.totalChecks}</div>
                          <p className="text-sm text-muted-foreground">Total Checks</p>
                        </div>
                        <div>
                          <div className="text-2xl font-bold text-green-600">{policy.passedChecks}</div>
                          <p className="text-sm text-muted-foreground">Passed</p>
                        </div>
                        <div>
                          <div className="text-2xl font-bold text-destructive">{policy.failedChecks}</div>
                          <p className="text-sm text-muted-foreground">Failed</p>
                        </div>
                      </div>
                      <Separator />
                      <div>
                        ...
                        <ScrollArea className="h-[200px]">
                          <div className="space-y-2">
                            {policyChecks
                              .filter((check) => check.policyId === policy.id && check.status === 'fail')
                              .map((check) => (
                                    ...
                                    <div className="flex items-start justify-between">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          <h5 className="font-semibold text-sm">{check.checkName}</h5>
                                        ...
                                        {check.affectedAssets.length > 0 && (
                                          <p className="text-xs text-muted-foreground mt-2">
                                            Affected: {check.affectedAssets.join(', ')}
                                          </p>
                                        )}
                                        {check.evidence && (
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Threat detection and quarantine management</CardDescription>
                </div>
            ...
            <CardContent>
              ...
              {malwareDetections.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No Malware Detected</h3>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-1 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="destructive">{detection.malwareType}</Badge>
                            </div>
                            <h4 className="font-semibold text-lg">Threat Detected: {detection.threatFamily}</h4>
                            <p className="text-sm text-muted-foreground">File: {detection.filePath}</p>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              <span>Asset: {detection.assetName}</span>
                              <span>Detected: {formatDate(detection.detectedDate)}</span>
                            </div>
                          </div>
                        </div>
                      </CardHeader>
        ...
        <TabsContent value="alerts" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Critical security notifications and incident management</CardDescription>
            </CardHeader>
            <CardContent>
              <img
                      ...
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="space-y-1 flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">{alert.description}</p>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              <span>Affected Assets: {alert.affectedAssets.join(', ')}</span>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Manage large-scale scanning across thousands of hosts and control groups</CardDescription>
                </div>
            ...
            <CardContent>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="async-scan">Asynchronous Scanning</Label>
                      </div>
                      <Checkbox
                        id="async-scan"
                        checked={asyncScanEnabled}
                        onCheckedChange={(checked) => setAsyncScanEnabled(checked as boolean)}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="perf-opt">Performance Optimization</Label>
                      </div>
                      <Checkbox
                        id="perf-opt"
                        checked={performanceOptimization}
                        onCheckedChange={(checked) => setPerformanceOptimization(checked as boolean)}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between">
                      <span className="text-sm text-muted-foreground">Total Assets Managed:</span>
                      <span className="font-bold">{totalAssetsInGroups.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-muted-foreground">Control Groups:</span>
                      <span className="font-bold">{totalControlGroups}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-muted-foreground">Active Scans:</span>
                      <span className="font-bold">{scanConfigs.filter((s) => s.enabled).length}</span>
                    </div>
                    <div className="flex justify-between">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Organize assets into logical groups for targeted scanning</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {controlGroups.map((group) => (
                        <TableRow key={group.id}>
                          ...
                          <TableCell>{getSeverityBadge(group.scanPriority)}</TableCell>
                          <TableCell>
                            <div className="flex gap-2">
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleStartScan('Targeted Scan', group.name)}
                              >
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Structured Audit Evidence Retention</CardTitle>
                  <CardDescription>Comprehensive logging for compliance and audit readiness</CardDescription>
                </CardHeader>
                <CardContent>
                  <img
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Generate comprehensive reports with exportable PDF and Excel formats</CardDescription>
        </CardHeader>
        <CardContent>
          <img
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">High-level security overview</span>
            </Button>
            <Button
              variant="outline"
              className="h-auto flex-col items-start p-4"
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">Detailed findings and metrics</span>
            </Button>
            <Button
              variant="outline"
              className="h-auto flex-col items-start p-4"
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">Framework-specific assessment</span>
            </Button>
            <Button
              variant="outline"
              className="h-auto flex-col items-start p-4"
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">Internal policy compliance</span>
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage continuous monitoring and scheduled scans with performance optimization</CardDescription>
        </CardHeader>
        <CardContent>
          <img
      ...
      <Dialog open={showGapAnalysisDialog} onOpenChange={setShowGapAnalysisDialog}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Detailed gap analysis with remediation recommendations and effort estimates
            </DialogDescription>
          </DialogHeader>
                ...
                <Card key={gap.controlId}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <CardTitle className="text-base">{gap.controlName}</CardTitle>
                          {gap.status === 'pass' ? (
                            <Badge variant="secondary">
                        ...
                        <p className="text-sm text-muted-foreground">Control ID: {gap.controlId}</p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
                      <p className="text-sm font-medium mb-1">Gap Identified:</p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm font-medium">{gap.estimatedEffort}</p>
                      </div>
                      <Button size="sm" variant="outline">
      ...
      <Dialog open={showScalabilityDialog} onOpenChange={setShowScalabilityDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Enterprise Scalability Configuration</DialogTitle>
            <DialogDescription>
              Configure performance optimization and scanning parameters for large-scale deployments
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <Card>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowScalabilityDialog(false)}>
                Cancel
              </Button>
              <Button onClick={() => {
      ...
      {/* Policy Audit Dialog */}
      <Dialog open={showPolicyAuditDialog} onOpenChange={setShowPolicyAuditDialog}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Policy Management</DialogTitle>
            <DialogDescription>
              View and manage internal organizational policies and Qualys-style benchmarks
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {policyAudits.map((policy) => (
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-muted-foreground">
                      {policy.totalChecks} checks • {policy.passedChecks} passed • {policy.failedChecks} failed
                    </div>
                    <Button size="sm" variant="outline">
                      <Eye className="mr-2 h-4 w-4" />
                      View Details
