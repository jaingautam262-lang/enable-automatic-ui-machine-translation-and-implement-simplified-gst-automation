...
const DATA_TYPES = ['invoices', 'transactions', 'contacts', 'journal-entries', 'products'];
const EXPORT_FORMATS = ['csv', 'excel'];

const FIELD_OPTIONS: Record<string, string[]> = {
  invoices: ['invoiceNumber', 'clientName', 'amount', 'date', 'dueDate', 'status'],
...
const MOCK_EXPORT_DATA: Record<string, object[]> = {
  invoices: [
    { invoiceNumber: 'INV-2026-001', clientName: 'Acme Corp', amount: 4500, date: '2026-01-15', status: 'paid' },
    { invoiceNumber: 'INV-2026-002', clientName: 'Tech Ltd', amount: 2800, date: '2026-02-01', status: 'unpaid' },
  ],
  transactions: [
    { date: '2026-02-01', description: 'Sales Revenue', amount: 5000, category: 'income', type: 'sale' },
    { date: '2026-02-02', description: 'Office Supplies', amount: -450, category: 'expense', type: 'expense' },
  ],
  contacts: [
    { name: 'John Smith', email: 'john@acme.com', phone: '+1-555-0100', company: 'Acme Corp' },
    { name: 'Jane Doe', email: 'jane@tech.com', phone: '+1-555-0200', company: 'Tech Ltd' },
  ],
  'journal-entries': [
    { date: '2026-02-01', description: 'Sales Revenue', debit: 0, credit: 5000, account: 'Revenue' },
    { date: '2026-02-02', description: 'Office Supplies', debit: 450, credit: 0, account: 'Expenses' },
  ],
  products: [
    { name: 'Widget A', sku: 'WGT-001', price: 29.99, stockLevel: 150, description: 'Standard widget' },
    { name: 'Widget B', sku: 'WGT-002', price: 49.99, stockLevel: 75, description: 'Premium widget' },
  ],
};
...
function validateRow(row: Record<string, string>, mappings: ColumnMapping[], dataType: string): { status: 'valid' | 'error' | 'warning'; messages: string[] } {
  ...
  for (const mapping of mappings) {
    if (numericFields.includes(mapping.targetField) && row[mapping.fileColumn]) {
      if (isNaN(parseFloat(row[mapping.fileColumn]))) {
...
export default function ImportExportTab() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const autoMappings: ColumnMapping[] = headers.map(h => ({
        fileColumn: h,
        targetField: fields.find(f => f.toLowerCase() === h.toLowerCase()) || '',
      }));
  ...
  const handleValidate = () => {
    ...
    if (errors > 0) {
      ...
      toast.success(`Validation passed! ${warnings} warning(s). Ready to commit.`);
    }
  };
  ...
  const handleCommit = () => {
    ...
    const existing = JSON.parse(localStorage.getItem(`import_${importDataType}`) || '[]');
    const newData = validRows.map(r => r.data);
    localStorage.setItem(`import_${importDataType}`, JSON.stringify([...existing, ...newData]));
    setCommitted(true);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Mass import or export accounting data via CSV and Excel files</p>
      </div>
        ...
        <TabsContent value="import" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upload a CSV or Excel file and map columns to data fields</CardDescription>
            </CardHeader>
              ...
              {parsedHeaders.length > 0 && (
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {mappings.map((mapping, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <span className="text-xs bg-muted px-2 py-1 rounded font-mono flex-1 truncate">{mapping.fileColumn}</span>
                          <ArrowRight className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                          <Select
                            value={mapping.targetField}
                            onValueChange={v => setMappings(prev => prev.map((m, idx) => idx === i ? { ...m, targetField: v } : m))}
                          >
                            ...
                            <SelectContent>
                              ...
                              {(FIELD_OPTIONS[importDataType] || []).map(f => (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="rounded-lg border border-border/50 overflow-x-auto">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            {parsedHeaders.map(h => <TableHead key={h} className="text-xs">{h}</TableHead>)}
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                  ...
                  <div className="flex gap-2">
                    ...
                    {validationResults.length > 0 && errorCount === 0 && !committed && (
                ...
                <div className="space-y-3">
                  ...
                  {validationResults.filter(r => r.status !== 'valid').length > 0 && (
                    <div className="rounded-lg border border-border/50 overflow-hidden">
                      <Table>
                        ...
                        <TableBody>
                          {validationResults.filter(r => r.status !== 'valid').map(r => (
                            <TableRow key={r.index}>
                              <TableCell className="text-xs">Row {r.index}</TableCell>
                              <TableCell>
                                <Badge className={`text-[10px] ${r.status === 'error' ? 'bg-red-500/20 text-red-700 border-red-500/30' : 'bg-amber-500/20 text-amber-700 border-amber-500/30'}`}>
                                  {r.status}
                                </Badge>
                              </TableCell>
                  ...
                  {committed && (
                    <div className="rounded-lg bg-green-50 dark:bg-green-950/20 p-3 flex items-center gap-2">
                      ...
                      <p className="text-sm text-green-700 font-medium">Import committed successfully!</p>
                    </div>
                  )}
                </div>
              )}
        ...
        <TabsContent value="export" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Download your accounting data in CSV or Excel format</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-muted-foreground">
                  Preview: {(MOCK_EXPORT_DATA[exportDataType] || []).length} records available
                </p>
                <div className="rounded-lg border border-border/50 overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        {Object.keys((MOCK_EXPORT_DATA[exportDataType] || [{}])[0] || {}).map(h => (
                          <TableHead key={h} className="text-xs capitalize">{h}</TableHead>
                        ))}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {(MOCK_EXPORT_DATA[exportDataType] || []).map((row, i) => (
                        <TableRow key={i}>
              ...
              <div className="rounded-lg bg-muted/30 p-3">
                <p className="text-xs text-muted-foreground">
                  <strong>Tip:</strong> Exported files can be re-imported to perform mass updates. Ensure column headers match the expected field names.
                </p>
              </div>
            </CardContent>
          </Card>
