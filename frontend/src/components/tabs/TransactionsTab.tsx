...
import { useGetAllTransactions, useAddTransaction, useUpdateTransaction, useCreateJournalEntryFromTransaction } from '../../hooks/useQueries';
...
import { Plus, Edit, DollarSign, BookOpen } from 'lucide-react';
...
export default function TransactionsTab() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (!selectedTransaction || !formData.category || !formData.amount || !formData.description) {
      toast.error(t('Please fill in all required fields'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('Transaction updated successfully. Journal entry and ledger updated automatically.'));
  ...
  const handleCreateJournalEntry = async (transaction: any) => {
    ...
    try {
      ...
      toast.success(t('Journal entry created from transaction. Ledger and trial balance updated.'));
    } catch (error: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{t('Record and manage all business transactions with journal entry creation')}</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t('Record a new business transaction with voice input support')}</DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={TransactionType.Sale}>{t('Sale')}</SelectItem>
                    <SelectItem value={TransactionType.Purchase}>{t('Purchase')}</SelectItem>
                    <SelectItem value={TransactionType.Income}>{t('Other Income')}</SelectItem>
                    <SelectItem value={TransactionType.Expense}>{t('Expense')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <VoiceInput
                    onTranscript={(text) => setFormData({ ...formData, category: text })}
                    size="icon"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <VoiceInput
                    onTranscript={(text) => setFormData({ ...formData, amount: text.replace(/[^0-9.]/g, '') })}
                    size="icon"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="associatedParty">{t('Associated Party')} ({t('Optional')})</Label>
                <div className="flex gap-2">
                  <Input
                    id="associatedParty"
                    placeholder={t('Customer or Supplier name')}
                    value={formData.associatedParty}
                    onChange={(e) => setFormData({ ...formData, associatedParty: e.target.value })}
                    className="flex-1"
                  />
                  <VoiceInput
                    onTranscript={(text) => setFormData({ ...formData, associatedParty: text })}
                    size="icon"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, isCash: checked as boolean })}
                />
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={addTransaction.isPending}>
                  {addTransaction.isPending ? t('Adding...') : t('Add Transaction')}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('All recorded transactions with edit support and journal entry creation')}</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">{t('Loading transactions...')}</div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">{t('Add your first transaction to get started')}</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {sortedTransactions.map((transaction) => (
                    <TableRow key={transaction.id.toString()}>
                      ...
                      <TableCell>{transaction.associatedParty || '-'}</TableCell>
                      <TableCell className="text-right font-medium">{formatINR(transaction.amount)}</TableCell>
                      <TableCell>
                        <Badge variant={transaction.isCash ? 'default' : 'secondary'}>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => handleCreateJournalEntry(transaction)}
                            title={t('Create Journal Entry')}
                            disabled={createJournalEntry.isPending || !!transaction.referenceId}
                          >
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('Update transaction details. Journal entry and ledger will be updated automatically.')}</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-associatedParty">{t('Associated Party')} ({t('Optional')})</Label>
              <div className="flex gap-2">
                <Input
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={updateTransaction.isPending}>
                {updateTransaction.isPending ? t('Updating...') : t('Update Transaction')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
