...
import { Download, FileJson, AlertCircle, CheckCircle2 } from 'lucide-react';
import { useI18n } from '../../i18n/useI18n';
import { useGenerateExport, useDownloadExport } from '../../hooks/useGSTExports';
import { GSPProvider } from '../../types/gst-universal';
import { ExportType } from '../../lib/gst/exports/types';
import { toast } from 'sonner';

export default function GSTExportsTab() {
  const { t } = useI18n();
  const [selectedProvider, setSelectedProvider] = useState<GSPProvider>('cleartax');
  const [selectedExportType, setSelectedExportType] = useState<ExportType>('einvoice');
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('gstExports.configurationDescription')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="provider">{t('gstExports.selectProvider')}</Label>
              <Select value={selectedProvider} onValueChange={(value) => setSelectedProvider(value as GSPProvider)}>
            ...
            <div className="space-y-2">
              <Label htmlFor="exportType">{t('gstExports.selectExportType')}</Label>
              <Select value={selectedExportType} onValueChange={(value) => setSelectedExportType(value as ExportType)}>
                <SelectTrigger id="exportType">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="einvoice">{t('gstExports.types.einvoice')}</SelectItem>
                  <SelectItem value="ewayBill">{t('gstExports.types.ewayBill')}</SelectItem>
                  <SelectItem value="gstr1">{t('gstExports.types.gstr1')}</SelectItem>
                  <SelectItem value="gstr3b">{t('gstExports.types.gstr3b')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {validationResult.errors.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-semibold text-destructive">{t('gstExports.errors')}</h4>
            ...
            {validationResult.warnings.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-semibold text-yellow-600">{t('gstExports.warnings')}</h4>
            ...
            {validationResult.isValid && validationResult.errors.length === 0 && (
              <Alert>
                ...
                <AlertDescription>{t('gstExports.validationPassed')}</AlertDescription>
              </Alert>
            )}
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('gstExports.selectedProvider')}:</span>
              <Badge>{selectedProvider}</Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('gstExports.selectedExportType')}:</span>
