...
import { useGetAllInvoices, useCreateInvoice, useUpdateInvoiceStatus, useGetNextInvoiceNumber } from '../../hooks/useQueries';
...
import { FileText, Plus, Printer, Info } from 'lucide-react';
...
export default function InvoicesTab() {
  ...
  const [items, setItems] = useState<InvoiceItem[]>([
    { productId: 0, description: '', quantity: 1, unitPrice: 0, total: 0, taxRate: effectiveTaxRate || 18, discount: 0 },
  ]);
  ...
  useEffect(() => {
    ...
    if (zeroRated) {
      ...
      setItems((prev) =>
        prev.map((item) => ({ ...item, taxRate: item.taxRate === 0 ? effectiveTaxRate || 18 : item.taxRate }))
      );
    }
  }, [gstScheme, isIndianContext]);

  const addItem = () => {
    const taxRateForNewItem = isIndianContext && isZeroRatedScheme(gstScheme) ? 0 : effectiveTaxRate || 18;
    setItems([
      ...items,
      { productId: 0, description: '', quantity: 1, unitPrice: 0, total: 0, taxRate: taxRateForNewItem, discount: 0 },
    ]);
  };
  ...
  const calculateTotals = () => {
    ...
    let totalTax = 0;
    if (isIndianContext && gstScheme === 'Composition') {
      totalTax = compositionLevy;
    } else {
      totalTax = items.reduce((sum, item) => sum + (item.total * item.taxRate) / 100, 0);
    }
  ...
  const handleCreateInvoice = async () => {
    try {
      ...
      if (isIndianContext) {
        ...
        if (gstScheme === 'Export_WithPayment' || gstScheme === 'Export_WithoutPayment') {
          invoice.lutBondReference = lutBondReference;
        }
        if (gstScheme === 'SEZ_WithPayment' || gstScheme === 'SEZ_WithoutPayment') {
  ...
  const resetForm = () => {
    ...
    setClientContact('');
    setClientAddress('');
    setItems([{ productId: 0, description: '', quantity: 1, unitPrice: 0, total: 0, taxRate: effectiveTaxRate || 18, discount: 0 }]);
    setPaymentTerms('');
    setNotes('');
    setGstin('');
    setCustomerGSTIN('');
  ...
  const getGstSchemeBadge = (scheme?: GSTScheme) => {
    ...
    const colorMap: Record<GSTScheme, string> = {
      Regular: '',
      Composition: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',
      Export_WithPayment: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
      Export_WithoutPayment: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
      SEZ_WithPayment: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
      SEZ_WithoutPayment: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
      DeemedExport: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
    };
    return (
      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${colorMap[scheme]}`}>
  ...
  const showStandardTax = !isIndianContext || gstScheme === 'Regular' || gstScheme === 'DeemedExport';
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {t('invoices.nextInvoiceNumber')}: <strong>{nextInvoiceNumber}</strong>
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={businessContact} onChange={(e) => setBusinessContact(e.target.value)} />
                </div>
              </div>
              <div className="space-y-2">
              ...
              {isIndianContext && (
                <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-4">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-sm">Invoice Type / GST Scheme</h3>
                    <Info className="h-4 w-4 text-muted-foreground" />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="gst-scheme">GST Scheme</Label>
                    <Select value={gstScheme} onValueChange={(v) => setGstScheme(v as GSTScheme)}>
                  ...
                  {(gstScheme === 'Export_WithPayment' || gstScheme === 'Export_WithoutPayment') && (
                    <div className="space-y-2">
                      <Label htmlFor="lut-bond">LUT / Bond Reference Number</Label>
                      <Input
                        id="lut-bond"
                      ...
                      <p className="text-xs text-muted-foreground">
                        Enter the Letter of Undertaking or Bond reference number for zero-rated export supply.
                      </p>
                    </div>
                  )}

                  {(gstScheme === 'SEZ_WithPayment' || gstScheme === 'SEZ_WithoutPayment') && (
                    <div className="space-y-2">
                      <Label htmlFor="sez-unit">SEZ Unit / Developer Name</Label>
                      <Input
                        id="sez-unit"
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        Specify the SEZ unit or developer to whom the supply is being made.
                      </p>
                    </div>
                  )}
                  ...
                  {gstScheme === 'Composition' && (
                    <div className="space-y-2">
                      <Label htmlFor="composition-levy">Composition Levy Amount (₹)</Label>
                      <Input
                        id="composition-levy"
                        type="number"
                        min={0}
                        value={compositionLevy}
                        onChange={(e) => setCompositionLevy(parseFloat(e.target.value) || 0)}
                        placeholder="0.00"
                      />
                      <p className="text-xs text-muted-foreground">
                        Flat composition levy applicable. Individual CGST/SGST/IGST lines are not shown.
                      </p>
                    </div>
                  )}
                  ...
                  {gstScheme === 'DeemedExport' && (
                    <div className="flex items-start gap-2 rounded-md bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-3">
                      ...
                      <p className="text-xs text-green-700 dark:text-green-300">
                        <strong>Deemed Export:</strong> Full CGST/SGST/IGST breakdown applies. The invoice will be
                        marked as a Deemed Export supply as per Section 147 of the CGST Act.
                      </p>
                    </div>
                  )}
                  ...
                  {showZeroRatedNote && (
                    <div className="flex items-start gap-2 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3">
                      ...
                      <p className="text-xs text-blue-700 dark:text-blue-300">
                        <strong>Zero-Rated Supply:</strong> Tax rate has been set to 0% for all line items. This is a
              ...
              <div className="border-t pt-4">
                <h3 className="font-semibold mb-2">{t('invoices.clientDetails')}</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>
                      ...
                      {isIndianContext && gstScheme === 'DeemedExport' && (
                        <span className="ml-1 text-xs text-green-600 dark:text-green-400">(Deemed Export)</span>
                      )}
                      :
                    </span>
              ...
              <Button onClick={handleCreateInvoice} disabled={createInvoice.isPending} className="w-full">
                {createInvoice.isPending ? t('common.creating') : t('invoices.createInvoice')}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
            ...
            <TableBody>
              {invoices.map((invoice: any) => (
                <TableRow key={invoice.id}>
                  ...
                  {isIndianContext && (
                    <TableCell>
                      {invoice.gstScheme ? getGstSchemeBadge(invoice.gstScheme) : (
