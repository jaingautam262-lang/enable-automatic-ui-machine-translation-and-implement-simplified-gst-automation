...
import { Plus, Download, FileSpreadsheet, Info, AlertCircle, Calculator, Edit2, Trash2 } from 'lucide-react';
...
export default function TDSTCSManagementTab() {
  const [tdsTransactions, setTdsTransactions] = useState<TDSTransaction[]>(() => {
    return safeLocalStorage.getItem<TDSTransaction[]>('tdsTransactions', []) || [];
  });
  
  const [tcsTransactions, setTcsTransactions] = useState<TCSTransaction[]>(() => {
    return safeLocalStorage.getItem<TCSTransaction[]>('tcsTransactions', []) || [];
  });
  ...
  const handleAddTDS = () => {
      ...
      const updated = tdsTransactions.map(t => 
        t.id === editingTDS.id 
          ? {
              ...
              amount,
              tdsRate: TDS_SECTIONS.find(s => s.section === tdsFormData.section)?.rate || 0,
              tdsAmount,
    ...
    setTdsFormData({ section: '', description: '', amount: '', panNumber: '', certificateNumber: '' });
  };
      ...
      const updated = tcsTransactions.map(t => 
        t.id === editingTCS.id 
          ? {
              ...
              amount,
              tcsRate: TCS_SECTIONS.find(s => s.section === tcsFormData.section)?.rate || 0,
              tcsAmount,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Tax Deducted at Source and Tax Collected at Source compliance</p>
        </div>
      </div>
                  ...
                  <Dialog open={isTDSDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>Record a TDS deduction</DialogDescription>
                      </DialogHeader>
                  ...
                  <TableBody>
                    {tdsTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell className="font-medium">{transaction.section}</TableCell>
                        <TableCell>{transaction.description}</TableCell>
                        <TableCell className="text-right">{formatINR(transaction.amount)}</TableCell>
                        ...
                        <TableCell>
                          <Badge variant={transaction.status === 'filed' ? 'default' : 'secondary'}>
                            {transaction.status}
                          </Badge>
                        </TableCell>
                  ...
                  <Dialog open={isTCSDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>Record a TCS collection</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => setTcsFormData({ ...tcsFormData, buyerPAN: e.target.value.toUpperCase() })}
                            maxLength={10}
                          />
                        </div>
                      </div>
                  ...
                  <TableBody>
                    {tcsTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell className="font-mono text-sm">{transaction.buyerPAN}</TableCell>
                        <TableCell>
                          <Badge variant={transaction.status === 'deposited' ? 'default' : 'secondary'}>
                            {transaction.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
