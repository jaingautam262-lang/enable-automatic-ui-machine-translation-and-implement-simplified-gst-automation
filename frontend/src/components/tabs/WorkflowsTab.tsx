...
import { Workflow, Play, Plus, Edit, Trash2, CheckCircle2, XCircle } from 'lucide-react';
...
export default function WorkflowsTab() {
  ...
  const createWorkflow = useCreateWorkflow(principal);
  ...
  const handleRunWorkflow = (workflowId: string) => {
    runWorkflow.mutate(workflowId, {
      ...
      onError: (error: any) => toast.error(error.message || t('workflows.runFailed')),
    });
  };
  ...
  const handleToggleEnabled = (workflow: WorkflowDefinition) => {
    ...
    updateWorkflow.mutate(
      { id: workflow.id, updates: { enabled: !workflow.enabled } },
      {
        ...
        onError: (error: any) => toast.error(error.message || t('workflows.updateFailed')),
      }
    );
  };
  ...
  const handleDelete = (workflow: WorkflowDefinition) => {
    ...
    if (confirm(t('workflows.confirmDelete'))) {
      deleteWorkflow.mutate(workflow.id, {
        ...
        onError: (error: any) => toast.error(error.message || t('workflows.deleteFailed')),
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('workflows.availableWorkflowsDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">{t('common.loading')}</div>
          ) : workflows.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">{t('workflows.noWorkflows')}</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('workflows.recentRunsDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          {workflowRuns.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">{t('workflows.noRuns')}</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {workflowRuns.slice(0, 10).map((run) => {
                  const duration = run.finishedAt
                    ...
                    : '-';
                  const completedSteps = run.stepLogs.filter((s) => s.status === 'success').length;
                  ...
                  return (
                    <TableRow key={run.id}>
                      ...
                      <TableCell>
                        ...
                        {run.status === 'running' && <Badge variant="outline">Running</Badge>}
                        {run.status === 'partial' && <Badge variant="secondary">Partial</Badge>}
                      </TableCell>
                      <TableCell>
