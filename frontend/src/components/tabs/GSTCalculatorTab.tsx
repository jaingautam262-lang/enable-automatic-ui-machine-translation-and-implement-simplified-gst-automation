...
export default function GSTCalculatorTab() {
  ...
  const calculateCompositionGST = () => {
    const turnover = parseFloat(amount) || 0;
    const rate = parseFloat(compositionRate) || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={scheme} onValueChange={(v) => setScheme(v as 'regular' | 'composition')}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="regular">{t('gstCalculator.regularScheme')}</TabsTrigger>
          <TabsTrigger value="composition">{t('gstCalculator.compositionScheme')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="regular">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('gstCalculator.regularDescription')}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="border-t pt-4 space-y-2">
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.baseAmount')}:</span>
                  <span className="font-medium">₹{regularResult.baseAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.cgst')} ({(regularResult.rate / 2).toFixed(2)}%):</span>
                  <span className="font-medium">₹{regularResult.cgst.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.sgst')} ({(regularResult.rate / 2).toFixed(2)}%):</span>
                  <span className="font-medium">₹{regularResult.sgst.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.totalGST')}:</span>
                  <span className="font-medium">₹{regularResult.gstAmount.toFixed(2)}</span>
                </div>
              ...
              <div className="bg-muted p-4 rounded-md space-y-2">
                <h4 className="font-semibold text-sm">{t('gstCalculator.regularFeatures')}</h4>
        ...
        <TabsContent value="composition">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('gstCalculator.compositionDescription')}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="turnover">{t('gstCalculator.annualTurnover')}</Label>
                <Input
                  id="turnover"
                  type="number"
                  value={amount}
              ...
              <div className="space-y-2">
                <Label htmlFor="compositionRate">{t('gstCalculator.compositionRate')}</Label>
                <Input
                  id="compositionRate"
                  type="number"
              ...
              <div className="border-t pt-4 space-y-2">
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.annualTurnover')}:</span>
                  <span className="font-medium">₹{compositionResult.turnover.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm">{t('gstCalculator.compositionRate')}:</span>
              ...
              <div className="bg-muted p-4 rounded-md space-y-2">
                <h4 className="font-semibold text-sm">{t('gstCalculator.compositionFeatures')}</h4>
                <ul className="text-sm space-y-1 list-disc list-inside">
