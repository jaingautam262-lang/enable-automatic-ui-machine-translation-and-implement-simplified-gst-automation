...
import { useGetAllDepreciationAssets, useAddDepreciationAsset, useUpdateDepreciationAsset, useCalculateDepreciation } from '../../hooks/useQueries';
import { formatCurrency, formatDate } from '../../lib/formatters';
import { Calculator, Plus, TrendingDown, FileText, Edit } from 'lucide-react';
...
export default function DepreciationReportsTab() {
  ...
  const openEditDialog = (asset: DepreciationAsset) => {
    ...
    setAcquisitionDate(new Date(Number(asset.acquisitionDate) / 1000000).toISOString().split('T')[0]);
  ...
  const handleCalculateDepreciation = async (assetId: bigint) => {
    try {
      const result = await calculateDepreciation.mutateAsync(assetId);
      toast.success(`Depreciation calculated: ${formatCurrency(result.depreciation)}`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Depreciation Reports</h2>
          <p className="text-muted-foreground">Asset depreciation tracking and calculations with editing support</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track depreciation for all registered assets</CardDescription>
        </CardHeader>
        <CardContent>
          {assets.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-2">Add your first asset to start tracking depreciation</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {assets.map((asset) => (
                  <TableRow key={asset.id.toString()}>
                    ...
                    <TableCell>{asset.depreciationMethod.toUpperCase()}</TableCell>
                    <TableCell className="text-right">{formatCurrency(asset.acquisitionCost)}</TableCell>
                    <TableCell className="text-right">{formatCurrency(asset.bookValue)}</TableCell>
                    <TableCell className="text-right">{formatCurrency(asset.accumulatedDepreciation)}</TableCell>
                    <TableCell>{formatDate(asset.lastCalculationDate)}</TableCell>
                    <TableCell>
      ...
      <Dialog open={showAddForm} onOpenChange={setShowAddForm}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Register a new asset for depreciation tracking</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddAsset} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="acquisitionCost">Acquisition Cost (INR) *</Label>
                <Input
                  id="acquisitionCost"
                  type="number"
                  step="0.01"
              ...
              <div className="space-y-2">
                ...
                <Select value={depreciationMethod} onValueChange={(v) => setDepreciationMethod(v as 'slm' | 'wdv')}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <Dialog open={showEditForm} onOpenChange={setShowEditForm}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Update asset details and recalculate depreciation</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditAsset} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-acquisitionCost">Acquisition Cost (INR) *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-residualValue">Residual Value (INR) *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-depreciationMethod">Depreciation Method *</Label>
                <Select value={depreciationMethod} onValueChange={(v) => setDepreciationMethod(v as 'slm' | 'wdv')}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-depreciationRate">Depreciation Rate (%) *</Label>
                <Input
