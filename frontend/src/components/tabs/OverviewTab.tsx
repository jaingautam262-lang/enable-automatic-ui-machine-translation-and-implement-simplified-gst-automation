import { useGetDashboardMetrics, useGetLowStockProducts, useGetOverdueInvoices } from '../../hooks/useQueries';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { Badge } from '@/components/ui/badge';
import { DollarSign, TrendingUp, TrendingDown, Package, AlertTriangle, FileText } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          {lowStockProducts.length > 0 && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {lowStockProducts.length} product{lowStockProducts.length > 1 ? 's are' : ' is'} running low on
          ...
          {overdueInvoices.length > 0 && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {overdueInvoices.length} invoice{overdueInvoices.length > 1 ? 's are' : ' is'} overdue. Review in
                the Invoices tab.
              </AlertDescription>
            </Alert>
          )}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          title="Stock Value"
          value={formatCurrency(metrics?.currentStockValue || 0)}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{formatCurrency(metrics?.outstandingReceivables || 0)}</div>
            {overdueInvoices.length > 0 && (
        ...
        <div className="relative h-48 bg-gradient-to-r from-primary/10 via-primary/5 to-background">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Track, analyze, and grow your business</p>
            </div>
          </div>
        </div>
      </Card>
