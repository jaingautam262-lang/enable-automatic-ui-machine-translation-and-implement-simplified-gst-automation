...
import { TrendingUp, TrendingDown, ArrowRightLeft, Calculator, Printer, Info } from 'lucide-react';
...
export default function FundFlowCalculatorTab() {
  ...
  const calculateFundFlow = () => {
    ...
    const sourcesData = {
      capitalIntroduced: parseFloat(sources.capitalIntroduced) || 0,
      loansReceived: parseFloat(sources.loansReceived) || 0,
      retainedEarnings: parseFloat(sources.retainedEarnings) || 0,
      assetSales: parseFloat(sources.assetSales) || 0,
      otherSources: parseFloat(sources.otherSources) || 0,
    };

    const applicationsData = {
      assetPurchases: parseFloat(applications.assetPurchases) || 0,
      loanRepayments: parseFloat(applications.loanRepayments) || 0,
      dividendsPaid: parseFloat(applications.dividendsPaid) || 0,
      operatingExpenses: parseFloat(applications.operatingExpenses) || 0,
      otherApplications: parseFloat(applications.otherApplications) || 0,
    };

    const currentAssetsIncrease = parseFloat(workingCapital.currentAssetsIncrease) || 0;
    const currentLiabilitiesIncrease = parseFloat(workingCapital.currentLiabilitiesIncrease) || 0;
    const netWorkingCapitalChange = currentAssetsIncrease - currentLiabilitiesIncrease;
  ...
  const generateFundFlowReportHTML = (record: FundFlowData) => {
    const totalSources = Object.values(record.sourcesOfFunds).reduce((sum, val) => sum + val, 0);
    const totalApplications = Object.values(record.applicationOfFunds).reduce((sum, val) => sum + val, 0);
  ...
  const totalSources = latestRecord ? Object.values(latestRecord.sourcesOfFunds).reduce((sum, val) => sum + val, 0) : 0;
  const totalApplications = latestRecord ? Object.values(latestRecord.applicationOfFunds).reduce((sum, val) => sum + val, 0) : 0;
  const netFundFlow = totalSources - totalApplications;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Analyze sources and applications of funds with working capital changes and visual charts</p>
        </div>
      </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          A fund flow statement shows the movement of funds into and out of a business, including changes in working capital.
          It helps analyze how funds were sourced and where they were applied during a specific period.
        </AlertDescription>
      </Alert>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSources({ ...sources, capitalIntroduced: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSources({ ...sources, loansReceived: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSources({ ...sources, retainedEarnings: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSources({ ...sources, assetSales: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSources({ ...sources, otherSources: e.target.value })}
              />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setApplications({ ...applications, assetPurchases: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setApplications({ ...applications, loanRepayments: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setApplications({ ...applications, dividendsPaid: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setApplications({ ...applications, operatingExpenses: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setApplications({ ...applications, otherApplications: e.target.value })}
              />
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="currentAssetsIncrease">Increase in Current Assets (₹)</Label>
              <Input
                id="currentAssetsIncrease"
                type="number"
                placeholder="0"
                value={workingCapital.currentAssetsIncrease}
                onChange={(e) => setWorkingCapital({ ...workingCapital, currentAssetsIncrease: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="currentLiabilitiesIncrease">Increase in Current Liabilities (₹)</Label>
              <Input
                id="currentLiabilitiesIncrease"
                type="number"
                placeholder="0"
                value={workingCapital.currentLiabilitiesIncrease}
                onChange={(e) => setWorkingCapital({ ...workingCapital, currentLiabilitiesIncrease: e.target.value })}
              />
            </div>
          </div>

          <Button onClick={calculateFundFlow} disabled={isCalculating} className="w-full">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => handlePrint(latestRecord)}>
                <Printer className="h-4 w-4 mr-2" />
                Print
              </Button>
            </CardTitle>
            <CardDescription>Generated on {latestRecord.date.toLocaleDateString()}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold mb-3 text-green-600">Sources of Funds</h3>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Capital Introduced</span>
                    <span className="font-medium">{formatINR(latestRecord.sourcesOfFunds.capitalIntroduced)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Loans Received</span>
                    <span className="font-medium">{formatINR(latestRecord.sourcesOfFunds.loansReceived)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Retained Earnings</span>
                    <span className="font-medium">{formatINR(latestRecord.sourcesOfFunds.retainedEarnings)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Asset Sales</span>
                    <span className="font-medium">{formatINR(latestRecord.sourcesOfFunds.assetSales)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Other Sources</span>
                    <span className="font-medium">{formatINR(latestRecord.sourcesOfFunds.otherSources)}</span>
                  </div>
              ...
              <div>
                <h3 className="font-semibold mb-3 text-red-600">Application of Funds</h3>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Asset Purchases</span>
                    <span className="font-medium">{formatINR(latestRecord.applicationOfFunds.assetPurchases)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Loan Repayments</span>
                    <span className="font-medium">{formatINR(latestRecord.applicationOfFunds.loanRepayments)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Dividends Paid</span>
                    <span className="font-medium">{formatINR(latestRecord.applicationOfFunds.dividendsPaid)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Operating Expenses</span>
                    <span className="font-medium">{formatINR(latestRecord.applicationOfFunds.operatingExpenses)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Other Applications</span>
                    <span className="font-medium">{formatINR(latestRecord.applicationOfFunds.otherApplications)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-2 font-bold">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatINR(latestRecord.workingCapitalChanges.currentAssetsIncrease)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Increase in Current Liabilities</span>
                    <span className="font-medium">{formatINR(latestRecord.workingCapitalChanges.currentLiabilitiesIncrease)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-2 font-bold">
                    <span>Net Working Capital Change</span>
                    <span>{formatINR(latestRecord.workingCapitalChanges.netWorkingCapitalChange)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span className={netFundFlow >= 0 ? 'text-green-600' : 'text-red-600'}>
                    {formatINR(netFundFlow)}
                  </span>
                </div>
              </div>
            ...
            <Table>
              ...
              <TableBody>
                {fundFlowRecords.slice(1).map((record) => {
                  const sources = Object.values(record.sourcesOfFunds).reduce((sum, val) => sum + val, 0);
                  const applications = Object.values(record.applicationOfFunds).reduce((sum, val) => sum + val, 0);
                  const net = sources - applications;
                  
                  return (
                    <TableRow key={record.id}>
                      <TableCell>{record.date.toLocaleDateString()}</TableCell>
                      <TableCell className="text-right">{formatINR(sources)}</TableCell>
                      <TableCell className="text-right">{formatINR(applications)}</TableCell>
