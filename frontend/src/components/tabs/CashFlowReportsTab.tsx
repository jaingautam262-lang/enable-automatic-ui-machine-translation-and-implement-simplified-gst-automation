  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('cashFlow.netCashFlow')}</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {netCashFlow >= 0 ? t('cashFlow.positiveCashFlow') : t('cashFlow.negativeCashFlow')}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('cashFlow.analysisTitle')} - {period.charAt(0).toUpperCase() + period.slice(1)}
          </CardTitle>
          ...
          {transactions.length === 0 ? (
            ...
            <div className="space-y-4">
              <div className="border rounded-lg p-4">
                <h3 className="font-semibold mb-2">{t('cashFlow.recentTransactions')}</h3>
                <div className="space-y-2">
                  {transactions.slice(0, 10).map((transaction) => (
                    <div key={transaction.id.toString()} className="flex justify-between items-center">
                      <div>
                        <p className="font-medium">{transaction.description}</p>
                        <p className="text-sm text-muted-foreground">{formatDate(transaction.date, language)}</p>
                      </div>
