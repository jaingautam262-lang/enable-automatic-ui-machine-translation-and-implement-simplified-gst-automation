...
import { fetchPersistedTranslations, normalizePersistedTranslations } from './persistedTranslations';
...
export function usePersistedTranslationsBootstrap(): UsePersistedTranslationsBootstrapReturn {
  ...
  const [persistedTranslations, setPersistedTranslations] = useState<Map<string, Map<string, string>>>(
    new Map()
  );
