 ...
 */
async function translateWithMyMemory(options: TranslateOptions): Promise<TranslateResponse> {
...
async function translateWithLibreTranslate(
  ...
  const { sourceText, targetLanguage, sourceLanguage = 'en' } = options;
  const baseUrl = options.baseUrl || settings.baseUrl || 'https://libretranslate.com';
...
async function translateWithAzure(
  ...
  const endpoint = settings.baseUrl || 'https://api.cognitive.microsofttranslator.com';
...
async function translateWithDeepL(
  ...
  const endpoint = settings.baseUrl || 'https://api-free.deepl.com/v2/translate';
...
export function saveProviderSettings(settings: ProviderSettings): void {
  try {
    localStorage.setItem('translation_provider_settings', JSON.stringify(settings));
  } catch (error) {
