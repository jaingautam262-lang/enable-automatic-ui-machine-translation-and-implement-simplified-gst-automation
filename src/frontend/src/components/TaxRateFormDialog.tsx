...
export default function TaxRateFormDialog({
  open,
  onOpenChange,
  editingRate,
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: open is intentional to reset form when dialog reopens
  useEffect(() => {
    if (editingRate) {
      setForm({
  ...
  const validate = () => {
    ...
    const pct = Number.parseFloat(form.percentage);
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {editingRate
              ...
              : "Fill in the details to create a new tax rate entry."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={form.taxType}
              onValueChange={(v) =>
                setForm((f) => ({ ...f, taxType: v as TaxRateType }))
              }
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              value={form.rateName}
              onChange={(e) =>
                setForm((f) => ({ ...f, rateName: e.target.value }))
              }
              className={errors.rateName ? "border-destructive" : ""}
            />
            {errors.rateName && (
              <p className="text-xs text-destructive">{errors.rateName}</p>
            )}
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Use this rate as the default for the selected country & type
              </p>
            </div>
            <Switch
