...
import { Eye, Play, Plus, Trash2, TrendingDown } from "lucide-react";
...
const INITIAL_DEFERRED: DeferredItem[] = [
  ...
  {
    id: "d2",
    name: "Prepaid Consulting Revenue",
    type: "revenue",
    totalAmount: 36000,
    startDate: "2026-01-01",
    months: 12,
...
function calcDepreciation(asset: AssetRecord): DepreciationRow[] {
  ...
  for (let year = 1; year <= asset.usefulLife; year++) {
    ...
    if (asset.method === "slm") {
      ...
      const rate =
        1 - (asset.residualValue / asset.cost) ** (1 / asset.usefulLife);
      dep = remaining * rate;
    }
    accumulated += dep;
    remaining -= dep;
...
export default function AssetsTab() {
  ...
  const [journalEntries, setJournalEntries] = useState<JournalEntryForEdit[]>([
    ...
    },
    {
      id: "je2",
      date: "2026-02-01",
      description: "Depreciation - Company Vehicle",
      amount: 17500,
      selected: false,
    },
    {
      id: "je3",
      date: "2026-02-01",
      description: "Depreciation - Server Infrastructure",
      amount: 20000,
      selected: false,
    },
  ]);
  ...
  const addAsset = () => {
    ...
    const asset: AssetRecord = {
      ...
      name: newAsset.name,
      cost: Number.parseFloat(newAsset.cost),
      purchaseDate:
        newAsset.purchaseDate || new Date().toISOString().split("T")[0],
      method: newAsset.method,
      usefulLife: Number.parseInt(newAsset.usefulLife) || 5,
      residualValue: Number.parseFloat(newAsset.residualValue) || 0,
      currentValue: Number.parseFloat(newAsset.cost),
    };
  ...
  const applyCutoff = () => {
    ...
    const n = Number.parseInt(splitPeriods) || 3;
  ...
  const addDeferredItem = () => {
    if (!newDeferred.name || !newDeferred.totalAmount) {
    ...
    const item: DeferredItem = {
      ...
      totalAmount: Number.parseFloat(newDeferred.totalAmount),
      startDate:
        newDeferred.startDate || new Date().toISOString().split("T")[0],
      months: Number.parseInt(newDeferred.months) || 12,
      recognized: 0,
    };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Asset tracking, depreciation schedules, deferred items, and cut-off
          tools
        </p>
      </div>
        ...
        <TabsContent value="assets" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track all fixed assets with depreciation parameters
              </CardDescription>
            </CardHeader>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Generate amortization journal entries for all assets
                  </CardDescription>
                </div>
            ...
            <CardContent>
              {journalEntries.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Click "Run Depreciation" to generate entries
                  </p>
                </div>
              ) : (
                <Table>
        ...
        <TabsContent value="deferred" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage multi-year contracts and prepaid items with recognition
                schedules
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewDeferred((p) => ({ ...p, name: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1">
        ...
        <TabsContent value="cutoff" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select journal entries and split amounts across multiple periods
                (one-shot deferred)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <TableRow>
                    <TableHead className="w-10">
                      <Checkbox
                        checked={
                          journalEntries.length > 0 &&
                          journalEntries.every((e) => e.selected)
                        }
                        onCheckedChange={(v) =>
                          setJournalEntries((prev) =>
                            prev.map((e) => ({ ...e, selected: !!v })),
                          )
                        }
                      />
                    </TableHead>
              ...
              <p className="text-xs text-muted-foreground">
                {journalEntries.filter((e) => e.selected).length} of{" "}
                {journalEntries.length} entries selected. Splitting will replace
                selected entries with {splitPeriods} equal period entries each.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
