...
import { ChevronRight, Edit2, Plus, Save, Trash2 } from "lucide-react";
import React, { useState } from "react";
import { toast } from "sonner";

class AnalyticErrorBoundary extends React.Component<
...
> {
  ...
  componentDidCatch(error: Error) {
    console.error("[AnalyticAccountingTab] Error:", error);
  }
  render() {
        ...
        <div className="flex flex-col items-center justify-center min-h-[300px] gap-4 text-center p-8">
          ...
          <div>
            <p className="text-lg font-semibold">
              Analytic Accounting failed to load
            </p>
...
function AnalyticAccountingContent() {
  ...
  const [plans, setPlans] = useState<AnalyticPlan[]>([
    ...
    },
    {
      ...
      applicability: "Project invoices",
    },
  ]);
  ...
  const addAccount = () => {
    ...
    setAccounts((prev) => [
      ...prev,
      { ...newAccount, id: Date.now().toString(), balance: 0 },
    ]);
  ...
  const applyBulkUpdate = () => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.selected
          ? { ...i, amount: Number.parseFloat(bulkAmount), selected: false }
          : i,
      ),
    );
    setBulkAmount("");
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Cost account hierarchies, analytic plans, budget management, and mass
          edit
        </p>
      </div>
        ...
        <TabsContent value="accounts" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Organize accounts in parent-child hierarchies by project,
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewAccount((p) => ({ ...p, code: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1">
        ...
        <TabsContent value="plans" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage multiple analytic plans with sub-plans and applicability
                conditions
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        <TabsContent value="budgets" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Compare budgeted amounts with actual performance per account
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {budgets.map((b) => {
                    ...
                    const pct =
                      b.budgeted > 0
                        ? ((b.actual / b.budgeted) * 100).toFixed(1)
                        : "0";
                    ...
                    return (
                      <TableRow key={b.accountId}>
                        ...
                        <TableCell className="text-sm text-right">
                          <Badge
                            className={`text-[10px] ${isOver ? "bg-red-500/20 text-red-700 border-red-500/30" : "bg-green-500/20 text-green-700 border-green-500/30"}`}
                          >
                            {pct}%
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Button
                            ...
                            variant="ghost"
                            className="h-6 w-6"
                            onClick={() => setEditingBudget(b.accountId)}
                          >
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <CardTitle className="text-base">
                    Mass Edit Analytic Items
                  </CardTitle>
                  <CardDescription>
                    Select multiple items and apply bulk updates
                  </CardDescription>
                </div>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {items.filter((i) => i.selected).length} of {items.length} items
                selected
              </p>
            </CardContent>
          </Card>
