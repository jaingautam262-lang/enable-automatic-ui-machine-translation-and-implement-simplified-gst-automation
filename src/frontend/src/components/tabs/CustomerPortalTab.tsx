...
export default function CustomerPortalTab() {
  ...
  const handlePayNow = (invoice: CustomerInvoice) => {
    toast.info(
      `Redirecting to payment for ${invoice.number} ($${invoice.amount})...`,
    );
  ...
  const handleUpgradeSubscription = (_id: string) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          View your invoices, manage subscriptions, and track orders
        </p>
      </div>
          ...
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">
                $
                {MOCK_INVOICES.filter((i) => i.status !== "paid")
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        className={`text-[10px] capitalize ${statusColors[sub.status]}`}
                      >
                        {sub.status}
                      </Badge>
                    </div>
                    ...
                    {(
                      ...
                      return (
                        <div key={step} className="flex items-center flex-1">
                          <div
                            className={`h-2 w-2 rounded-full shrink-0 ${isCompleted ? "bg-primary" : "bg-muted"}`}
                          />
                          {i < 3 && (
                            <div
                              className={`h-0.5 flex-1 ${stepIdx < currentIdx ? "bg-primary" : "bg-muted"}`}
                            />
                          )}
                        </div>
                      );
