...
import {
  useGetLegacyTaxSettings,
  useSetLegacyTaxSettings,
} from "../../hooks/useLegacyTaxSettings";
import {
...
import { COUNTRY_TAX_RATES, getCurrencyForCountry } from "../../lib/taxRates";
...
export default function SettingsTab() {
  ...
  const { selectedCountry, setSelectedCountry: _setSelectedCountry } =
    useSelectedTaxCountry();
  ...
  // Wrap setSelectedCountry to also dispatch currency sync event
  const setSelectedCountry = (code: string) => {
    _setSelectedCountry(code);
    window.dispatchEvent(new Event("currencyCountryChange"));
  };

  // Manual currency override state
  const [manualSymbol, setManualSymbol] = useState<string>(() => {
    return localStorage.getItem("manualCurrencySymbol") || "";
  });

  // Keep manualSymbol in sync when context clears override
  useEffect(() => {
    if (!manualOverride) {
      setManualSymbol("");
    }
  ...
  // Derived currency info for the selected country (auto-detected, not override)
  const autoDetectedCurrency = getCurrencyForCountry(selectedCountry);

  const [localWebsiteUrl, setLocalWebsiteUrl] = useState(
    branding.websiteURL || "",
  );
  const [localQrDataUrl, setLocalQrDataUrl] = useState(
    branding.paymentQRDataURL || "",
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {t("settings.autoTranslationNote")}
        </p>
      </div>
      ...
      <Tabs defaultValue="tax" className="w-full">
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="currency" data-ocid="settings.currency_tab">
            Currency
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="tax" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure your tax settings and GST information
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-1">
                  ...
                  >
                    {autoDetectedCurrency.symbol} — {autoDetectedCurrency.name}{" "}
                    ({autoDetectedCurrency.code})
                  </Badge>
                  {manualOverride && (
                    <Badge
                      variant="outline"
                      className="text-xs px-2 py-0.5 text-amber-600 border-amber-300"
                    >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleLegacySettingsChange("gstin", e.target.value)
                  }
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleLegacySettingsChange("gstType", value)
                  }
                >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleLegacySettingsChange("stateCode", e.target.value)
                  }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    handleNotificationChange("overdueInvoiceAlert", checked)
                  }
                />
              </div>
            </CardContent>
        ...
        <TabsContent value="translation" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mymemory">
                      {t("settings.translation.myMemory")}
                    </SelectItem>
                    <SelectItem value="libretranslate">
                      {t("settings.translation.libreTranslate")}
                    </SelectItem>
                    <SelectItem value="google">
                      {t("settings.translation.googleTranslate")}
                    </SelectItem>
                    <SelectItem value="azure">
                      {t("settings.translation.azureTranslate")}
                    </SelectItem>
                    <SelectItem value="deepl">
                      {t("settings.translation.deepL")}
                    </SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-sm text-muted-foreground">
                  {providerSettings.provider === "mymemory" &&
                    t("settings.translation.myMemoryNote")}
                  {providerSettings.provider === "libretranslate" &&
                    t("settings.translation.libreTranslateNote")}
                  {providerSettings.provider === "google" &&
                    t("settings.translation.googleNote")}
                  {providerSettings.provider === "azure" &&
                    t("settings.translation.azureNote")}
                  {providerSettings.provider === "deepl" &&
                    t("settings.translation.deeplNote")}
                </p>
              </div>
              ...
              {providerSettings.provider === "libretranslate" && (
                <div className="space-y-2">
                  <Label htmlFor="baseUrl">
                    {t("settings.translation.baseUrl")}
                  </Label>
                  <Input
                    id="baseUrl"
              ...
              {["google", "azure", "deepl", "libretranslate"].includes(
              ...
              <Button onClick={handleProviderSave}>
                {t("settings.translation.saveProvider")}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="currency" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Currency symbol is automatically set based on your selected
                country. You can override it manually for custom display.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
                ...
                <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2.5">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {autoDetectedCurrency.code} ·{" "}
                      {autoDetectedCurrency.locale}
                    </span>
                  </div>
                  {manualOverride ? (
                    <Badge
                      variant="outline"
                      className="ml-auto text-xs text-amber-600 border-amber-300 bg-amber-50 dark:bg-amber-950/20"
                    >
                      Overridden by: {manualOverride}
                    </Badge>
                  ) : (
                    <Badge
                      variant="secondary"
                      className="ml-auto text-xs text-green-700 bg-green-100 dark:bg-green-900/30 dark:text-green-400"
                    >
                      Active
                    </Badge>
                  )}
              ...
              <div className="space-y-2">
                <Label htmlFor="manualCurrencySymbol">
                  Manual Override Symbol
                </Label>
                <Input
                  id="manualCurrencySymbol"
                  data-ocid="currency.override_input"
                  value={manualSymbol}
                  onChange={(e) => setManualSymbol(e.target.value)}
                  placeholder={`e.g. ${autoDetectedCurrency.symbol} or $`}
                  className="max-w-xs"
                  maxLength={10}
                />
                <p className="text-xs text-muted-foreground">
                  Enter a custom symbol to display instead of the auto-detected
                  one. Leave blank and press "Reset to Auto" to revert.
                </p>
              </div>
              ...
              <div className="rounded-lg border border-dashed border-border bg-muted/30 px-4 py-3 space-y-1">
                ...
                <p className="text-xl font-bold tabular-nums">
                  <span className="text-primary">
                    {manualSymbol || currencySymbol}
                  </span>
                  {autoDetectedCurrency.code === "INR"
                    ? "1,23,456.00"
                    : "1,234,567.89"}
                </p>
                <p className="text-xs text-muted-foreground">
                  This is how amounts will appear across the dashboard and
                  features.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 pt-1">
                <Button
                  data-ocid="currency.save_button"
                  onClick={() => {
                    if (manualSymbol.trim()) {
                      setManualOverride(manualSymbol.trim());
                      toast.success(
                        `Currency symbol overridden to "${manualSymbol.trim()}"`,
                      );
                    } else {
                      toast.error("Please enter a symbol to save.");
                    }
                  }}
                >
                  Save Override
                ...
                <Button
                  variant="outline"
                  data-ocid="currency.reset_button"
                  onClick={() => {
                    clearManualOverride();
                    setManualSymbol("");
                    toast.success("Currency reset to auto-detected symbol.");
                  }}
                >
                  Reset to Auto
                </Button>
