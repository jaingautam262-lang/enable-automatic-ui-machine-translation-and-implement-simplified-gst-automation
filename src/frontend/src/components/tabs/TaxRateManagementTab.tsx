...
export default function TaxRateManagementTab() {
  ...
  const [businessGstRegistered, setBusinessGstRegistered] = useState(true);
  ...
  const countries = useMemo(() => {
    ...
    for (const r of taxRates) {
      map.set(r.countryCode, r.country);
    }
  ...
  const filteredRates = useMemo(() => {
    return taxRates.filter((r) => {
      ...
      const matchCountry =
        filterCountry === "all" || r.countryCode === filterCountry;
  ...
  const groupedByCountry = useMemo(() => {
    const groups = new Map<
      string,
      { code: string; name: string; rates: TaxRate[] }
    >();
    ...
    <div className="space-y-6">
      ...
      <Card className="border-border/60">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Indicate whether your business is registered under GST. This affects
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label
                htmlFor="business-gst-switch"
                className="text-sm font-medium cursor-pointer"
              >
                My Business is GST Registered
              </Label>
              <p className="text-xs text-muted-foreground">
                Enable if you have a valid GSTIN and collect GST from customers
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Switch
                id="business-gst-switch"
                checked={businessGstRegistered}
                onCheckedChange={setBusinessGstRegistered}
                data-ocid="tax.gst_registered_switch"
              />
            </div>
          </div>
          ...
          {!businessGstRegistered && (
            <div className="flex items-start gap-2 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-3">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300">
                <strong>GST is disabled</strong> — invoices will be issued as
                Tax Exempt (0% tax). Enable if you are registered under GST.
              </p>
            </div>
          )}
          ...
          <div className="pt-1">
            <Button
              ...
              onClick={() => {
                toast.info("Use the form to add a 0% Tax Exempt rate.", {
                  description:
                    "Click the 'Add Tax Rate' button, set percentage to 0, and name it 'Tax Exempt'.",
                });
                handleAdd();
              }}
              data-ocid="tax.add_exempt_button"
            >
              <Plus className="h-3.5 w-3.5" />
              Add Tax Exempt (0%) Rate
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage GST, VAT, and Income Tax rates for all countries. Add, edit,
            or delete any rate.
          </p>
        </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <CardDescription className="text-xs">
                            {group.rates.length} rate
                            {group.rates.length !== 1 ? "s" : ""}
                          </CardDescription>
                        </div>
                      </div>
                      <div className="flex gap-1.5 flex-wrap justify-end">
                        {Array.from(
                          new Set(group.rates.map((r) => r.taxType)),
                        ).map((type) => (
                          <span
                            key={type}
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${TAX_TYPE_COLORS[type]}`}
                          >
                            {type}
                          </span>
                        ))}
                          ...
                          {group.rates.map((rate) => (
                            ...
                            >
                              ...
                              <TableCell className="py-2.5">
                                <span
                                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${TAX_TYPE_COLORS[rate.taxType]}`}
                                >
                                  {rate.taxType}
                                </span>
                              </TableCell>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              be undone. Predefined rates can be re-seeded by clearing your
              browser data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
