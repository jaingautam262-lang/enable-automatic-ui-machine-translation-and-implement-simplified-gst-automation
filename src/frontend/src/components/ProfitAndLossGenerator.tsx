...
import {
  useGenerateProfitAndLossStatement,
...
export default function ProfitAndLossGenerator() {
  const { data: plStatements = [], isLoading: loadingStatements } =
    useGetAllProfitAndLossStatements();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const administrativeExpenses: OperatingExpense[] =
        formData.administrativeExpenses
          .filter((item) => item.name && item.amount)
          .map((item, idx) => ({
            id: BigInt(idx + 1),
            name: item.name,
      ...
      const result = await generatePLMutation.mutateAsync({
        ...
        depreciation: Number.parseFloat(formData.depreciation),
        otherIncome,
        operatingExpenses,
        administrativeExpenses,
        financialExpenses,
      ...
      <body>
        ...
        <table>
          ...
          <tbody>
            ...
            ${statement.administrativeExpenses
              .map(
                (exp) => `
              <tr>
                <td>&nbsp;&nbsp;${exp.name}</td>
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <Dialog open={isGenerateOpen} onOpenChange={setIsGenerateOpen}>
            <Button
              ...
              disabled={isGenerating || tradingAccounts.length === 0}
            >
            ...
            <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select a trading account and add expenses to generate P&L
                  statement
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {tradingAccounts.map((ta) => (
                      <option
                        key={bigIntToString(ta.id)}
                        value={bigIntToString(ta.id)}
                      >
                        {formatDate(ta.date)} - Gross Profit:{" "}
                        {formatINR(ta.grossProfit)}
                      </option>
                    ))}
                  </select>
                </div>
                      ...
                      <div key={index} className="grid grid-cols-2 gap-2 mb-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              removeItem("operatingExpenses", index)
                            }
                            disabled={isGenerating}
                          >
                            ×
                      ...
                      <div key={index} className="grid grid-cols-2 gap-2 mb-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              removeItem("administrativeExpenses", index)
                            }
                            disabled={isGenerating}
                          >
                            ×
                      ...
                      <div key={index} className="grid grid-cols-2 gap-2 mb-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              removeItem("financialExpenses", index)
                            }
                            disabled={isGenerating}
                          >
                            ×
        ...
        {loadingStatements ? (
                ...
                <CardContent className="pt-6">
                  <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.grossProfit)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.totalOtherIncome)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.totalOperatingExpenses)}
                          </span>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.totalAdministrativeExpenses)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.totalFinancialExpenses)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {formatINR(statement.depreciation)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-3 border-t-2 border-primary/20">
                      ...
                      <span
                        className={`font-bold text-lg ${statement.netProfit >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
          ...
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-2">
              Generate a Trading Account first, then create P&L statement
            </p>
            {tradingAccounts.length === 0 && (
              <Alert className="mt-4 text-left">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  <strong>Note:</strong> You need to generate a Trading Account
                  before creating a P&L statement.
                </AlertDescription>
              </Alert>
            )}
          </div>
