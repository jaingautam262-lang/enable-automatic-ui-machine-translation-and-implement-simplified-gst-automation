...
class TranslationCache {
    ...
    try {
      ...
      for (const key of keys) {
        if (key.startsWith(CACHE_KEY_PREFIX + CACHE_VERSION)) {
          const language = key.replace(
            `${CACHE_KEY_PREFIX + CACHE_VERSION}_`,
            "",
          );
          ...
          if (data) {
            try {
              ...
              console.warn(
                `Failed to parse cache for language ${language}:`,
                error,
              );
            }
          }
  ...
  private saveToLocalStorage(language: string, langCache: LanguageCache): void {
    try {
      ...
      console.warn(
        `Failed to save cache for language ${language} to localStorage:`,
        error,
      );
    }
  }
